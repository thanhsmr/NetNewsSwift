...
extension CAPSPageMenu {
    ...
    func topRightButtonTouch() {
        self.delegate?.topRightButtonTouch!()
    }
    ...
    func setUpUserInterface() {
        ...
        menuScrollView.frame = CGRect(x: 0.0, y: 0.0, width: self.view.frame.width - self.rightBottomWith, height: configuration.menuHeight)
        
        self.view.addSubview(menuScrollView)
        
        let topRightButton = UIButton.init(frame: CGRect.init(x: menuScrollView.frame.size.width, y: 0, width: self.rightBottomWith, height: configuration.menuHeight))
        topRightButton.setImage(UIImage.init(named: "ic_top_right_page_menu"), for: UIControlState.normal)
        topRightButton.addTarget(self, action: #selector(topRightButtonTouch), for: .touchUpOutside)
    ...
    func configureUserInterface() {
        ...
        if configuration.useMenuLikeSegmentedControl {
            ...
            menuScrollView.contentSize = CGSize(width: (configuration.menuItemWidth + configuration.menuMargin) * CGFloat(controllerArray.count) + configuration.menuMargin + self.rightBottomWith, height: configuration.menuHeight)
        }
        ...
        if configuration.menuItemWidthBasedOnTitleTextWidth {
            menuScrollView.contentSize = CGSize(width: (totalMenuItemWidthIfDifferentWidths + configuration.menuMargin) + CGFloat(controllerArray.count) * configuration.menuMargin + self.rightBottomWith, height: configuration.menuHeight)
        }
