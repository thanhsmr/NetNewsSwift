...
class MenuItemView: UIView {
    ...
    func setUpMenuFirstImageItemView(_ menuItemWidth: CGFloat, menuScrollViewHeight: CGFloat, indicatorHeight: CGFloat, separatorPercentageHeight: CGFloat, separatorWidth: CGFloat, separatorRoundEdges: Bool, menuItemSeparatorColor: UIColor, viewControllerTag: Int) {
        let imagePage = UIImageView.init(frame: CGRect(x: 0.0, y: 10.0, width: menuItemWidth, height: 20))
        imagePage.contentMode = UIViewContentMode.scaleAspectFit
        titleLabel = UILabel(frame: CGRect.init(x: 0, y: 0, width: 0, height: 0))
        
        switch viewControllerTag {
        case 0:
            imagePage.image = UIImage.init(named: "ic_top_left_news")
        case 1:
            imagePage.image = UIImage.init(named: "ic_top_left_tv")
        case 2:
            imagePage.image = UIImage.init(named: "ic_top_left_radio")
    ...
    func configure(for pageMenu: CAPSPageMenu, controller: UIViewController, index: CGFloat) {
        if pageMenu.configuration.useMenuLikeSegmentedControl {
            ...
            if index == 0 {
                self.setUpMenuFirstImageItemView(pageMenu.configuration.menuItemWidth, menuScrollViewHeight: pageMenu.configuration.menuHeight, indicatorHeight: pageMenu.configuration.selectionIndicatorHeight, separatorPercentageHeight: pageMenu.configuration.menuItemSeparatorPercentageHeight, separatorWidth: pageMenu.configuration.menuItemSeparatorWidth, separatorRoundEdges: pageMenu.configuration.menuItemSeparatorRoundEdges, menuItemSeparatorColor: pageMenu.configuration.menuItemSeparatorColor, viewControllerTag: controller.view.tag)
            } else {
